---
description: "Summarize the current session in the background using projmem MCP tools"
triggers:
  - summarize
  - session summary
  - save summary
...
Call `projmem_session_list` with `{ "status": "active", "limit": 1 }`.
...
Call `projmem_session_transcript` with `{ "sessionId": "<id from step 1>" }`.
Read the full transcript carefully.

### Step 3 — Generate and save the summary

Analyze the transcript and call `projmem_session_update_summary` with:
